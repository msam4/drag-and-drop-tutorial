...
const isTouchDevice = () => {
  try{
    // Create a TouchEvent(it would fail for deskstops and throw error)
...
const drop = (e) => {
  ...
  if (isTouchDevice()) {
    ...
    if(e.target.id == "drop-point"){
      ...
      dragObject.classList.add("hide");
      e.target.insertAdjacentHTML("afterbegin", '<div id="draggable-object"></div>')
    }
  }
}
...
window.onload = async () => {
  ...
  dragObject.addEventListener("touched", drop);
